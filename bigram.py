...
n_head = 6 # how many attention heads to use inside the transformer
n_layers = 6 # how many transformer blocks to use in the architecture
dropout = 0.2 # how much dropout to apply in the model
# ------------
...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        wei = wei.masked_fill(self.tril[:T,:T] == 0, float('-inf')) # mask out the upper half of the matrix # (B,T,T)
...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        pos_emb = self.positional_embedding_table(torch.arange(idx.shape[1], device=device)) # (T,C)
    ...
    def generate(self, idx, max_new_tokens):
        ...
        for _ in range(max_new_tokens):
            # crop id to the indices in the current context
